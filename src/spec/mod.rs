use std::{any::{Any, TypeId}, marker::PhantomData, result, os::raw, string, collections::HashMap};
...
use crate::conn::xcp::STIMTimeoutMode;
...
mod test {
    ...
    use super::{A2ML, NamedNode, IfData, Blob};
    #[test]
    fn hello_childs(){
        let mut x = A2ML {
        ...
        let x = x.find_child_by_name::<Blob>("blob").map(|z| {
            z.model_link.clone()
        }).unwrap();
        assert_eq!(x,"link")
    }
...
impl A2LFile {
    fn get_node_by_name<T: 'static + NamedNode>(&self, name: &str)->Option<&T>{
        self.project.descendants(&self.internal).find(|x| {
            self.internal[*x].get().downcast_ref::<T>().map_or(None, |y| {
                if y.name() == name {
                    Some(1)
                } else {
                    None
            ...
            self.internal[z].get().downcast_ref::<T>()
        })
    }
    fn get_node<T: 'static + Node>(&self)->Option<&T> {
        self.project.descendants(&self.internal).find(|x| {
            self.internal[*x].get().downcast_ref::<T>().is_some()
        }).map_or(None, |z| {
            self.internal[z].get().downcast_ref::<T>()
        })
    }
    ...
    fn get_nodes<T: 'static + Node>(&self)->Vec<&T> {
        self.project.descendants(&self.internal).filter(|x| {
            self.internal[*x].get().downcast_ref::<T>().is_some()
        }).map(|y| {
            self.internal[y].get().downcast_ref::<T>().unwrap()
        }).collect()
    }
    ...
    fn get_node_mut_by_name<T: 'static + NamedNode>(&mut self, name: &str)->Option<&mut T>{
        self.project.descendants(&self.internal).find(|x| {
            ...
            self.internal[z].get_mut().downcast_mut::<T>()
        })
    }
